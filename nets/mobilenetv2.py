...
class DeformableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
        ...
        self.stride = (stride, stride)  # Ensure stride is in tuple form
...
class MobileNetV2(nn.Module):
    def __init__(self, n_class=1000, input_size=224, width_mult=1., downsample_factor=8, pretrained=True):
    ...
    def _nostride_dilate(self, m, dilate):
        ...
        if classname.find('Conv') != -1 or classname.find('DeformableConv2d') != -1:
            if m.stride == (2, 2):
                m.stride = (1, 1)
